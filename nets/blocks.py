...
def get_patches(
    input_map: torch.Tensor, locations: torch.Tensor, patch_size: int = 3
):
    ...
    x_indices = x_coords.unsqueeze(-1).unsqueeze(-1) + x_grid.cpu()
    y_indices = y_coords.unsqueeze(-1).unsqueeze(-1) + y_grid.cpu()
...
class DeformableConv2d(nn.Module):
    ...
    def forward(self, x):
        ...
        max_offset = torch.tensor(max_offset, device="cpu")
