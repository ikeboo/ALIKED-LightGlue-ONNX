...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=int,
        required=True,
        help="Input image size format (e.g. 640)",
    )
    parser.add_argument(
        "--device",
        type=str,
...
def main():
    ...
    # export_aliked_to_onnx( "aliked_dynamic.onnx")
    ...
    # change dynamic axes to static axes
    model = onnx.load(args.model_output)
    onnx_session = onnxruntime.InferenceSession(args.model_output)
    inputs = {}
    for i in onnx_session.get_inputs():
        inputs[i.name] = [i.shape[0], i.shape[1], args.input_size, args.input_size]
    ...
    # save model
    onnx.save(model_simp, args.model_output)
